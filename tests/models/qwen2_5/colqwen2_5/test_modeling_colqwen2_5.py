...
def model_name() -> str:
    return "vidore/colqwen2_5-v0.1"


@pytest.fixture(scope="module")
def model_from_pretrained(model_name: str) -> Generator[ColQwen2_5, None, None]:
...
@pytest.mark.slow
def test_load_from_pretrained(model_from_pretrained: ColQwen2_5):
    assert isinstance(model_from_pretrained, ColQwen2_5)


@pytest.mark.slow
def test_forward_images(
    model_from_pretrained: ColQwen2_5,
...
@pytest.mark.slow
def test_forward_queries(model_from_pretrained: ColQwen2_5, processor: ColQwen2_5_Processor):
    queries = [
...
@pytest.mark.slow
def test_is_model_deterministic(model_from_pretrained: ColQwen2_5, processor: ColQwen2_5_Processor):
    queries = [
        "Is attention really all you need?",
        "Are Benjamin, Antoine, Merve, and Jo best friends?",
    ]
