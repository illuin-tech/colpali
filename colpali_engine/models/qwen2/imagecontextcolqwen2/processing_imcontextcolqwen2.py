...
class ImageContextColQwen2Processor(BaseVisualRetrieverProcessor, Qwen2VLProcessor):
    """
    ...
    visual_prompt_3_images: ClassVar[str] = (
    ...
    def encode_images_to_base64(self, images: List[Image.Image]) -> List[str]:
        encoded_images = []
        for image in images:
            buffered = BytesIO()
    ...
    def process_images(
        self, images: List[Image.Image], prev_images: List[Image.Image], next_images: List[Image.Image]
    ) -> BatchFeature:
        """
        Process images for ColQwen2.
        """
        assert len(images) == len(prev_images)
        ...
        texts_docs = [self.visual_prompt_3_images] * len(images)
        batch_doc = self(
            text=texts_docs,
            images=images_list,
            padding="longest",
