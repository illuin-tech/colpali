...
class ContextColQwen2Processor(BaseVisualRetrieverProcessor, Qwen2VLProcessor):
    """
    ...
    visual_prompt_prefix_with_summary: ClassVar[str] = (
        ...
        "It was extracted from a document, here is a summary of the entire document to put it into context :"
        "\n<|thisisasummary|><|im_end|><|endoftext|>"
    )
    ...
    def process_images(self, images: List[Image.Image], summaries: Optional[List[str]] = None) -> BatchFeature:
        """
        Process images for ColQwen2.
        """
        if summaries is None:
            texts_doc = [self.visual_prompt_prefix_no_summary] * len(images)
        else:
            assert len(images) == len(summaries)  # Ensure that the number of images and summaries match
            texts_doc = [
                self.visual_prompt_prefix_with_summary.replace("<|thisisasummary|>", summary) for summary in summaries
            ]
